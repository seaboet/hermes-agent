...
it('does not send local-app or non-OAuth catalog suggestions through the hosted OAuth-only composer flow', () => {
  const hosted = {
    name: 'hosted', url: 'https://mcp.example.test', auth_type: 'oauth', transport: 'http',
    requires_app: false,
    availability: { state: 'no_requirements', version: null, path: null, min_version: null },
    suggest: { keywords: ['hosted'], hosts: [], applications: [], examples: [] }
  }

  const editor = {
    ...hosted, name: 'editor', url: 'http://127.0.0.1:8000/mcp', auth_type: 'none', requires_app: true,
